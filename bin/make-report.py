...
def parse_read_hist_file(filepath):
    """Parse a .hist file generated by faster2 (value\tcount)"""
    data = []
    try:
        with open(filepath, 'r') as f:
            for line in f:
...
def render_svg_histogram(values, counts, title, x_label, y_label, color="#5f708b", height=150, width=400, sparkline=False, min_v_fixed=None, max_v_fixed=None, mark_values=None):
    ...
    svg = f'<svg viewBox="0 0 {width} {height}" preserveAspectRatio="xMidYMid meet" style="width: 100%; height: {height if sparkline else "auto"}; max-height: {height}px;">'
    ...
    return f"""
    <div class="histogram-container" style="background: white; border-radius: 8px; padding: 10px; border: 1px solid #e2e8f0;">
...
def render_read_hists_section(samples_readhists):
    ...
    for sample_name in sorted(samples_readhists.keys()):
        ...
        qual_spark = "No data"
        if 'qual' in hists:
            vals, counts = zip(*hists['qual'])
            qual_spark = render_svg_histogram(vals, counts, "", "Avg Q-Score", "Count", height=40, width=280, sparkline=True, min_v_fixed=1, max_v_fixed=60, mark_values=[10, 20, 30, 40, 50, 60])
            
        html += f"""
              <tr data-sample="{sample_name.lower()}">
                <td class="sample-col">{sample_name}</td>
                <td style="padding: 5px; vertical-align: middle;">{len_spark}</td>
                <td style="padding: 5px; vertical-align: middle;">{gc_spark}</td>
                <td style="padding: 5px; vertical-align: middle;">{qual_spark}</td>
              </tr>
        """
